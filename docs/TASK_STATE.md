...
### P1.0: Auth forms DaisyUI 5 compliance
- Status: **DONE (2026-04-12)**
- Login and register forms rewritten from legacy DaisyUI v4 patterns (form-control, label-text, label-text-alt, nested alert divs, focus:input-primary) to working DaisyUI 5 + Tailwind utility approach.
- Discovery: `.fieldset`/`.fieldset-legend` CSS not generated by DaisyUI 5 plugin; `.label` is hint-only (nowrap, dim). See KI-011.
- Solution: plain `<div>` field wrappers, `text-sm font-semibold` labels, `input w-full`/`select w-full` inputs, `text-xs text-error` errors, `flex flex-col gap-4` form layout.
